...
import { PROFILE_COVERS } from '../utils/constants'; // Constants'tan defaultu al

export default function ProfileCard({ user, onClose, stats, userScores }) {
  if (!user) return null;

  // KAPAK FOTOĞRAFI MANTIĞI:
  // 1. Kullanıcı seçmişse onu kullan.
  // 2. Seçmemişse varsayılan (ilk) kapağı kullan.
  const coverImage = user.coverImage || PROFILE_COVERS[0];

  // Rozet Hesaplama
  const earnedBadges = calculateUserBadges(userScores || []);
    ...
    <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={onClose}>
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden relative animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
            ...
            <div className="px-6 pb-8 -mt-12 relative">
                ...
                <div className="mt-3">
                    <h2 className="text-2xl font-bold text-slate-800">{user.realName || "İsimsiz"}</h2>
                    <p className="text-slate-500 font-medium text-sm">@{user.username || "kullanici"}</p>
                    {user.statusMessage && (
                ...
                <div className="grid grid-cols-3 gap-3 mt-6">
                    <div className="bg-orange-50 p-3 rounded-2xl text-center border border-orange-100">
                        ...
                        <div className="font-bold text-slate-800 text-sm">{stats?.bestRank || "-"}</div>
                    ...
                    <div className="bg-blue-50 p-3 rounded-2xl text-center border border-blue-100">
                        ...
                        <div className="font-bold text-slate-800 text-sm">{stats?.examCount || 0}</div>
                    ...
                    <div className="bg-purple-50 p-3 rounded-2xl text-center border border-purple-100">
                        ...
                        <div className="font-bold text-slate-800 text-xs pt-1 truncate">{stats?.lastExamDate || "-"}</div>
                    ...
                    <div className="mt-6">
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {showcaseBadges.map((badge, i) => (
                                <div key={i} className={`p-2 rounded-xl flex flex-col items-center gap-1 text-center shadow-sm border border-slate-100 bg-white`}>
                                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-white shadow-sm text-sm ${badge.color}`}>
                                        {badge.icon}
                                    </div>
                                    <div className="text-[9px] font-bold text-slate-700 leading-tight truncate w-full">{badge.title}</div>
                                </div>
                            ))}
                        </div>
                    </div>
