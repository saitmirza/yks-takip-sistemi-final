...
export default function Chat({ currentUser, usersList, onUserClick }) {
  ...
  return (
    // h-[calc(100dvh-9rem)] -> Mobil tarayıcı çubuğu sorununu çözer
    <div className="flex h-[calc(100dvh-9rem)] md:h-[calc(100vh-6rem)] bg-white dark:bg-slate-900/90 dark:backdrop-blur-md rounded-3xl shadow-xl border border-slate-200 dark:border-slate-700 overflow-hidden relative">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar">
            ...
            {usersList.filter(u => u.internalId !== currentUser.internalId && !u.isAdmin).map(user => (
                <button key={user.internalId} onClick={() => handleSelectChat('private', user)} className={`w-full p-3 rounded-2xl flex items-center gap-3 transition-all ${selectedPartner?.internalId === user.internalId ? 'bg-white dark:bg-slate-600 shadow-md border border-indigo-100 dark:border-slate-500' : 'hover:bg-slate-200 dark:hover:bg-slate-700/50'}`}>
                    <div className="relative"><div className="w-10 h-10 rounded-full bg-slate-200 dark:bg-slate-600 overflow-hidden flex items-center justify-center border border-slate-100 dark:border-slate-500">{user.base64Avatar ? <img src={user.base64Avatar} className="w-full h-full object-cover" /> : <span className="text-xl">{user.avatar}</span>}</div>{isUserOnline(user.lastSeen) && <div className="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white rounded-full"></div>}</div>
      ...
      <div className={`flex-1 flex flex-col relative ${!showChatScreen ? 'hidden md:flex' : 'flex'} bg-[#efeae2] dark:bg-[#0b141a]`}>
            ...
            <div className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-2 overflow-hidden">
                    <div>
                        ...
                        <p className="text-xs text-slate-500 dark:text-slate-400">{chatMode === 'public' ? <span>{usersList.length} Üye</span> : (isUserOnline(selectedPartner?.lastSeen) ? <span className="text-green-600 font-bold">Çevrimiçi</span> : 'Son görülme yok')}</p>
                    </div>
                </div>
            </div>
            ...
            {messages.map((msg) => {
                ...
                return (
                    <div key={msg.id} className={`flex items-end gap-2 mb-2 ${isMe ? 'flex-row-reverse' : ''} group`}>
                        ...
                        {/* BALONLAR: App.js global stilinden etkilenmemesi için !important kullanıldı */}
                        <div className={`max-w-[85%] md:max-w-[75%] px-3 py-2 rounded-2xl text-sm shadow-sm relative leading-relaxed 
                            ${isMe 
                                ? 'bg-[#d9fdd3] !bg-[#005c4b] text-slate-800 !text-white rounded-tr-none' // BEN (Koyu Yeşil)
                                : 'bg-white !bg-[#1f2937] text-slate-800 !text-white rounded-tl-none'      // KARŞI (Koyu Gri)
                            }`}>
                            ...
                            {(isMe || currentUser.isAdmin) && <button onClick={() => handleDelete(msg.id)} className="absolute -top-2 -right-2 bg-white dark:bg-slate-700 text-red-500 p-1 rounded-full opacity-0 group-hover:opacity-100 transition-opacity shadow-md border border-slate-100 dark:border-slate-600"><Trash2 size={12}/></button>}
                        </div>
                    </div>
                )
            })}
         ...
         <form onSubmit={handleSend} className="p-2 bg-[#f0f2f5] dark:bg-[#1f2937] flex gap-2 items-center relative safe-area-bottom pb-4 md:pb-2">
             <button type="button" onClick={() => setShowEmoji(!showEmoji)} className={`p-2 rounded-full transition-colors ${showEmoji ? 'bg-indigo-100 text-indigo-600' : 'text-slate-500 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'}`}><Smile size={24} /></button>
             {showEmoji && <div className="absolute bottom-16 left-2 bg-white dark:bg-slate-800 p-2 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-600 grid grid-cols-6 gap-1 animate-in slide-in-from-bottom-5 z-20 w-72">{emojis.map(e => <button key={e} type="button" onClick={() => addEmoji(e)} className="text-xl hover:bg-slate-100 dark:hover:bg-slate-700 p-1 rounded-lg transition-colors">{e}</button>)}</div>}
             
             {/* Input'un rengi App.js'den eziliyor ama burada da güvene alalım */}
             <input type="text" className="flex-1 bg-white dark:bg-slate-700 dark:text-white border-0 rounded-xl px-3 py-3 focus:ring-1 focus:ring-indigo-500 outline-none transition-all shadow-sm placeholder-slate-400 text-sm" placeholder="Mesaj..." value={inputText} onChange={(e) => setInputText(e.target.value)}/>
