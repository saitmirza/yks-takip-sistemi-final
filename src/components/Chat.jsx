...
import { Send, Users, MessageCircle, Trash2, MoreVertical, Smile, ArrowLeft } from 'lucide-react'; // ArrowLeft eklendi
...
export default function Chat({ currentUser, usersList, onUserClick }) {
  ...
  // Mobil Görünüm Kontrolü: Eğer bir partner seçiliyse (veya public chat açıksa) mobilde mesaj ekranını göster
  // Ancak başlangıçta mobilde liste görünsün istiyoruz. 
  // Bu yüzden 'showChatScreen' adında bir state ekleyelim.
  ...
  // Sohbet seçildiğinde mobilde ekranı değiştir
  const handleSelectChat = (mode, partner) => {
      setChatMode(mode);
      setSelectedPartner(partner);
      setShowChatScreen(true); // Mobilde sohbet ekranına geç
  };

  const handleBackToList = () => {
      setShowChatScreen(false); // Listeye geri dön
  };
  ...
  return (
    <div className="flex h-[calc(100vh-120px)] md:h-[calc(100vh-6rem)] bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden relative">
      
      {/* --- SOL PANEL (Kişi Listesi) --- */}
      {/* Mobilde 'showChatScreen' true ise gizle, değilse göster. Masaüstünde hep göster. */}
      <div className={`w-full md:w-80 bg-slate-50 border-r border-slate-200 flex flex-col ${showChatScreen ? 'hidden md:flex' : 'flex'}`}>
        <div className="p-4 border-b border-slate-200"><h2 className="font-bold text-slate-700 text-lg">Sohbetler</h2></div>
        <div className="flex-1 overflow-y-auto p-3 space-y-2 custom-scrollbar">
            <button onClick={() => handleSelectChat('public', null)} className={`w-full p-3 rounded-2xl flex items-center gap-3 transition-all ${chatMode === 'public' ? 'bg-indigo-600 text-white shadow-md' : 'hover:bg-slate-200 text-slate-700'}`}>
            ...
            {usersList.filter(u => u.internalId !== currentUser.internalId && !u.isAdmin).map(user => (
                <button key={user.internalId} onClick={() => handleSelectChat('private', user)} className={`w-full p-3 rounded-2xl flex items-center gap-3 transition-all ${selectedPartner?.internalId === user.internalId ? 'bg-white shadow-md border border-indigo-100' : 'hover:bg-slate-200'}`}>
      ...
      {/* Mobilde 'showChatScreen' false ise gizle. Masaüstünde hep göster. */}
      <div className={`flex-1 flex flex-col bg-[#efeae2] relative ${!showChatScreen ? 'hidden md:flex' : 'flex'}`}>
         <div className="p-3 bg-white border-b border-slate-200 flex items-center justify-between shadow-sm z-10">
            <div className="flex items-center gap-2">
                {/* Mobilde Geri Butonu */}
                <button onClick={handleBackToList} className="md:hidden p-2 -ml-2 text-slate-600 hover:bg-slate-100 rounded-full"><ArrowLeft size={20}/></button>
                
                <div className="hidden md:block"><MessageCircle size={20} className="text-slate-400"/></div>
                <div className="flex items-center gap-2 overflow-hidden">
                    {chatMode === 'private' && selectedPartner && <Avatar url={selectedPartner.base64Avatar || selectedPartner.avatar} size="w-8 h-8" />}
                    <div>
                        <h3 className="font-bold text-slate-800 cursor-pointer hover:underline truncate max-w-[150px] sm:max-w-md" onClick={() => selectedPartner && onUserClick && onUserClick(selectedPartner.internalId)}>
            ...
            {messages.map((msg) => {
                ...
                return (
                    <div key={msg.id} className={`flex items-end gap-2 mb-2 ${isMe ? 'flex-row-reverse' : ''} group`}>
                        ...
                        <div className={`max-w-[85%] md:max-w-[75%] px-3 py-2 rounded-2xl text-sm shadow-sm relative leading-relaxed ${isMe ? 'bg-[#d9fdd3] text-slate-800 rounded-tr-none' : 'bg-white text-slate-800 rounded-tl-none'}`}>
         ...
         <form onSubmit={handleSend} className="p-2 bg-[#f0f2f5] flex gap-2 items-center relative safe-area-bottom">
             <button type="button" onClick={() => setShowEmoji(!showEmoji)} className={`p-2 rounded-full transition-colors ${showEmoji ? 'bg-indigo-100 text-indigo-600' : 'text-slate-500 hover:bg-slate-200'}`}><Smile size={24} /></button>
             {showEmoji && (
                 <div className="absolute bottom-16 left-2 bg-white p-2 rounded-2xl shadow-2xl border border-slate-200 grid grid-cols-6 gap-1 animate-in slide-in-from-bottom-5 z-20 w-72">
                     {emojis.map(e => <button key={e} type="button" onClick={() => addEmoji(e)} className="text-xl hover:bg-slate-100 p-1 rounded-lg transition-colors">{e}</button>)}
                 </div>
             )}
             <input type="text" className="flex-1 bg-white border-0 rounded-xl px-3 py-3 focus:ring-1 focus:ring-indigo-500 outline-none transition-all shadow-sm placeholder-slate-400 text-sm" placeholder="Mesaj..." value={inputText} onChange={(e) => setInputText(e.target.value)}/>
