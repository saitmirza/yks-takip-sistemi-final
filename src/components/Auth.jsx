...
const InputField = ({ icon: Icon, type, placeholder, value, onChange, colSpan = "col-span-1" }) => (
  <div className={`relative ${colSpan}`}>
    ...
    <input
      type={type}
      className="block w-full pl-10 pr-3 py-3 border border-slate-600 rounded-xl leading-5 bg-slate-900/50 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all duration-200 sm:text-sm"
...
<div className="col-span-full">
    ...
    <select className="w-full p-3 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl outline-none text-slate-700 dark:text-white" value={authInput.classSection} onChange={e => setAuthInput({...authInput, classSection: e.target.value})}>
        ...
        <option>12-A</option><option>12-B</option><option>12-C</option><option>12-D</option><option>12-E</option><option>12-F</option><option>Mezun</option>
                ...
                <div className="pt-3 border-t border-slate-700">
                    ...
                    <div className="grid grid-cols-4 gap-2">
                        {[9, 10, 11, 12].map(grade => (
                            <input key={grade} type="number" placeholder={`${grade}.`} className="p-2 border border-slate-600 rounded-lg text-center text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-slate-900/50 text-white transition-colors" value={authInput[`s${grade}Avg`] || ""} onChange={e => setAuthInput({...authInput, [`s${grade}Avg`]: e.target.value})} />
                        ))}
                    </div>
                </div>
             </div>
           ...
           {authError && <div className="bg-red-900/30 text-red-400 text-sm p-3 rounded-xl flex items-center gap-2 border border-red-800 font-medium"><ShieldAlert size={16} /> {authError}</div>}
         ...
         <div className="mt-6 text-center">
           <button onClick={() => { setAuthMode(authMode === 'login' ? 'register' : 'login'); setAuthError(""); }} className="text-indigo-400 text-sm font-semibold hover:text-indigo-300 transition-colors">
