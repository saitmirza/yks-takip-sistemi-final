...
import { Calculator, Target, Zap, CheckCircle2 } from 'lucide-react';
import { calculateOBP, getEstimatedRank } from '../utils/helpers';

export default function Simulator({ currentUser }) {
  const [nets, setNets] = useState({
      tytMath: 40, tytTurk: 40, tytFen: 20, tytSos: 20,
      aytMath: 40, aytFiz: 14, aytKim: 13, aytBiyo: 13
  });
  ...
  const handleCalculate = async () => {
      // --- DOĞRU KATSAYILAR (SAYISAL YERLEŞTİRME PUANI İÇİN) ---
      // Kaynak: 2024 YKS Verileri (Yaklaşık Değerler)
      // Not: Bu katsayılar 500 ham puan üzerinden hesaplanır (100 taban puan hariç dağılım)

      // TYT (Toplam 160 Puan Katkısı)
      const c_tyt_mat = 1.32;   // Matematik
      const c_tyt_turk = 1.32;  // Türkçe
      const c_tyt_fen = 1.36;   // Fen
      const c_tyt_sos = 1.36;   // Sosyal

      // AYT (Toplam 240 Puan Katkısı - Sayısal)
      const c_ayt_mat = 3.00;   // Matematik
      const c_ayt_fiz = 2.85;   // Fizik
      const c_ayt_kim = 3.07;   // Kimya
      const c_ayt_biyo = 3.07;  // Biyoloji

      // 1. Ham Puan Hesaplama (Base 100 + Netler)
      // TYT ve AYT puanları ayrı ayrı değil, havuza toplanır.
      ...
      let finalScore = 100 + rawScoreFromNets; // 100 Taban Puan ekle
      if (finalScore > 500) finalScore = 500; // Maksimum 500 (Teorik sınır)

      // 2. Yerleştirme Puanı (Ham + OBP)
  ...
  const handleChange = (field, val) => {
      let v = Number(val);
      // Basit sınır kontrolleri
      if (field.includes('tyt') && v > 40) v = 40; // Sos/Fen normalde 20 ama kodda 40 kalsın hata vermesin
        ...
        <div className="w-full md:w-80 flex flex-col gap-4">
            <div className="bg-slate-800 text-white p-6 rounded-3xl shadow-lg relative overflow-hidden min-h-[220px] flex flex-col justify-between">
                ...
                <div>
                    ...
                    <div className="text-[10px] text-indigo-300 mt-1 font-medium bg-indigo-500/20 inline-block px-2 py-0.5 rounded">
                        OBP: +{obpData.placementBonus} Eklendi
                    </div>
                </div>
                ...
                <div className="pt-4 border-t border-white/10">
                    ...
                    <div className="text-xl font-bold text-green-400 leading-tight">
                        {result ? result.rank : "Hesaplanmadı"}
                    </div>
                </div>
            </div>
            ...
            {result && (
                <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm space-y-2 text-sm">
                    <div className="flex justify-between items-center pt-2">
                        <span className="font-bold text-slate-800">Ham Puan (OBP'siz)</span>
                        <span className="font-bold text-indigo-600 text-lg">{result.final}</span>
                    </div>
                    <div className="text-xs text-slate-400 text-center mt-2 bg-slate-50 p-2 rounded">
                        *2024 YKS katsayılarına göre hesaplanmıştır.
                    </div>
                </div>
            )}
        </div>
