...
const InputGroup = ({ label, value, onChange, max = 40, color = "focus:ring-indigo-500" }) => (
    <div className="bg-black/20 p-2 rounded-xl border border-white/5">
        ...
        <input 
          type="number" 
          className={`w-full bg-slate-900/80 border border-slate-700 rounded-lg p-2 text-center font-bold text-white outline-none focus:ring-2 ${color} placeholder-slate-600`}
          value={value}
...
export default function Simulator({ currentUser }) {
  ...
  return (
    <div className="max-w-4xl mx-auto flex flex-col md:flex-row gap-8 pb-20">
        ...
        {/* SOL: HESAPLAMA FORMU (GLASS FIX) */}
        <div className="flex-1 glass-box p-6 rounded-3xl shadow-xl transition-colors">
            <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
                <Calculator className="text-indigo-400"/> Sayısal Puan Hesapla
            </h2>
            
            <div className="space-y-6">
                <div>
                    <h3 className="text-xs font-bold text-indigo-400 mb-3 uppercase tracking-wider border-b border-indigo-500/30 pb-1 flex items-center gap-2">
                        <Zap size={14}/> TYT Netleri
                    </h3>
                ...
                <div>
                    <h3 className="text-xs font-bold text-purple-400 mb-3 uppercase tracking-wider border-b border-purple-500/30 pb-1 flex items-center gap-2">
                        <Target size={14}/> AYT Sayısal Netleri
                    </h3>
                    <div className="grid grid-cols-4 gap-2">
        ...
        <div className="w-full md:w-80 flex flex-col gap-4">
            <div className="bg-slate-800 text-white p-6 rounded-3xl shadow-lg relative overflow-hidden min-h-[200px] flex flex-col justify-between border border-slate-700">
                ...
                <div>
                    ...
                    <div className="text-[10px] text-indigo-300 mt-1 font-medium bg-indigo-500/20 inline-block px-2 py-0.5 rounded border border-indigo-500/30">OBP: +{obpData.placementBonus} Eklendi</div>
                </div>
            ...
            {result && (
                <div className="glass-box p-5 rounded-3xl shadow-sm space-y-2 text-sm">
                    <div className="flex justify-between items-center pt-2">
                        <span className="font-bold text-white">Ham Puan</span>
                        <span className="font-bold text-indigo-400 text-lg">{result.ham}</span>
                    </div>
                    <div className="text-xs text-slate-400 text-center mt-2 bg-white/5 p-2 rounded border border-white/5">
                        *2024 YKS Sayısal yığılma verilerine göre.
                    </div>
                </div>
            )}
        </div>
