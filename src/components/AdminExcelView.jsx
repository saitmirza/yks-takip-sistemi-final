...
export default function AdminExcelView({ usersList, allScores, appId, dataToEdit }) {
    ...
    useEffect(() => {
        ...
        // NORMAL MOD (Yeni veya Mevcut Seçimi)
        if (mode === 'create') {
    ...
    }, [usersList, mode, selectedExamId, allScores, dataToEdit, classFilter]); // classFilter bağımlılığı eklendi
    ...
    const handleDeleteExam = async () => {
        ...
        const confirmMsg = `DİKKAT! "${selectedExamId}" sınavına ait TÜM ÖĞRENCİ KAYITLARI kalıcı olarak silinecek.\n\nBu işlem geri alınamaz. Onaylıyor musun?`;
        if (!confirm(confirmMsg)) return;
        if (!confirm("Gerçekten emin misin? Son karar.")) return;
        ...
        try {
            ...
            await batch.commit();
            alert("Sınav silindi.");
            setMode('create'); setSelectedExamId("");
            setExamConfig({ name: '', date: new Date().toISOString().split('T')[0], type: 'both' });
        ...
        for (const row of rowsToProcess) {
            try {
                ...
                if (!row.manualTytScore && !row.manualAytScore) {
                    const nets = { 
                        tytMath: Number(row.tytMath), tytTurk: Number(row.tytTurk), 
                        tytFen: Number(row.tytFen), tytSos: Number(row.tytSos), 
                        aytMath: examConfig.type !== 'tyt' ? Number(row.aytMath) : 0, 
                        aytFen: examConfig.type !== 'tyt' ? Number(row.aytFen) : 0 
                    };
    ...
    return (
        <div className="max-w-[98%] mx-auto bg-white dark:bg-gray-800 rounded-3xl shadow-xl overflow-hidden border border-slate-100 dark:border-gray-700 pb-32 transition-colors relative">
            
            {/* ÜST BAR VE SINIF FİLTRESİ */}
            <div className="flex flex-col md:flex-row border-b border-slate-100 dark:border-gray-700 justify-between items-center pr-4 bg-slate-50/50 dark:bg-gray-800">
                ...
                {/* SINIF FİLTRESİ DROPDOWN */}
                <div className="flex items-center gap-2 p-2 w-full md:w-auto border-t md:border-t-0 md:border-l border-slate-200 dark:border-gray-600">
                    <Filter size={16} className="text-slate-400"/>
                    <select 
                        value={classFilter} 
                        onChange={(e) => setClassFilter(e.target.value)}
                        className="bg-white dark:bg-gray-700 border border-slate-200 dark:border-gray-600 text-slate-700 dark:text-white text-xs font-bold rounded-lg py-2 px-3 outline-none cursor-pointer hover:bg-slate-50"
                    >
                ...
                {mode === 'edit' && selectedExamId && (
                    <button onClick={handleDeleteExam} className="hidden md:flex bg-red-50 dark:bg-red-900/20 hover:bg-red-100 text-red-600 dark:text-red-400 px-4 py-2 rounded-xl text-xs font-bold items-center gap-2 transition-colors border border-red-200 dark:border-red-800 ml-4"><Trash2 size={16}/> Sınavı Sil</button>
                )}
            </div>
            
            {/* AYARLAR BAR */}
            <div className="p-4 md:p-8 bg-white dark:bg-gray-800 border-b border-slate-100 dark:border-gray-700 grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                {mode === 'create' ? (
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-sm text-left">
                    <thead className="text-xs text-slate-500 uppercase bg-slate-50/50 dark:bg-gray-800/50 border-b border-slate-100 dark:border-gray-700">
                        <tr>
                            <th className="px-6 py-4 font-bold w-48 text-slate-400 sticky left-0 bg-white dark:bg-gray-800 z-10">Öğrenci</th>
                            ...
                            {(examConfig.type !== 'tyt') && <><th className="px-2 text-center text-purple-400 font-bold min-w-[60px]">Mat</th><th className="px-2 text-center text-purple-400 font-bold min-w-[60px]">Fen</th><th className="px-2 text-center w-24 bg-purple-50/50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300 font-extrabold">AYT P</th></>}
                    ...
                    <tbody className="divide-y divide-slate-50 dark:divide-gray-700">
                        {rows.length > 0 ? rows.map((row) => (
                            <tr key={row.uid} className={`hover:bg-slate-50/80 dark:hover:bg-gray-700/50 transition-colors group ${row.status === 'saved' ? 'bg-green-50/50 dark:bg-green-900/20' : ''}`}>
                                <td className="px-6 py-3 font-semibold text-slate-700 dark:text-white truncate max-w-[12rem] sticky left-0 bg-white dark:bg-gray-800 z-10 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]">{row.name}{row.docId && mode === 'edit' && <span className="ml-2 text-[9px] bg-blue-100 text-blue-600 px-1 rounded">✓</span>}</td>
                        ...
                        )) : <tr><td colSpan="10" className="p-8 text-center text-slate-400">{classFilter !== 'ALL' ? `${classFilter} sınıfında öğrenci yok.` : 'Liste yükleniyor...'}</td></tr>}
                    </tbody>
                </table>
            </div>

            {/* KAYDET BAR */}
            <div className="p-4 md:p-6 bg-white dark:bg-gray-800 border-t border-slate-100 dark:border-gray-700 flex justify-between items-center fixed bottom-20 md:bottom-0 right-0 left-0 md:left-64 z-40 shadow-[0_-4px_20px_rgba(0,0,0,0.1)]">
                <div className="text-sm text-slate-500 dark:text-gray-400 font-medium">Toplam <span className="font-bold text-slate-800 dark:text-white">{rows.length}</span> öğrenci ({classFilter === 'ALL' ? 'Tümü' : classFilter})</div>
                <button onClick={handleSaveAll} disabled={isSaving} className="flex items-center gap-2 px-6 py-3 md:px-8 md:py-4 rounded-2xl font-bold text-white bg-indigo-600 hover:bg-indigo-700 shadow-xl transition-all disabled:opacity-70 disabled:cursor-not-allowed">
