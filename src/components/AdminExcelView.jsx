...
export default function AdminExcelView({ usersList, allScores, appId }) {
    ...
    const createEmptyRow = (user) => ({
        uid: user.internalId, docId: null, name: user.realName || user.username,
        tytMath: '', tytTurk: '', tytFen: '', tytSos: '', aytMath: '', aytFen: '', manualTytScore: '', manualAytScore: '', status: 'idle'
    });
    ...
    const handleSaveAll = async () => {
        ...
        for (const row of rowsToProcess) {
            try {
                // OTOMATİK HESAPLAMA (Eğer manuel giriş boşsa netlerden hesapla)
                let calcFinal = 0;
                if (!row.manualTytScore && !row.manualAytScore) {
                    const nets = {
                        ...
                        aytMath: Number(row.aytMath), aytFen: Number(row.aytFen) // AYT Fen toplu
                    };
                    const calculated = calculateScoreSAY(nets); // Helpers'daki fonksiyonu kullanır
                    calcFinal = calculated.say;
                } else {
                    // Manuel puan girilmişse onu baz al
                    calcFinal = (Number(row.manualTytScore) * 0.4) + (Number(row.manualAytScore) * 0.6);
                }
    ...
    return (
        <div className="max-w-[95%] mx-auto bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100 pb-20">
            <div className="flex border-b border-slate-100">
                <button onClick={() => { setMode('create'); setExamConfig({ name: '', date: new Date().toISOString().split('T')[0], type: 'both' }); setRows(rows.map(r => ({...r, status:'idle', docId: null, manualTytScore:'', manualAytScore:''}))); }} className={`flex-1 p-4 flex items-center justify-center gap-2 font-bold transition-colors ${mode === 'create' ? 'bg-indigo-50 text-indigo-600 border-b-2 border-indigo-600' : 'text-slate-500 hover:bg-slate-50'}`}><PlusCircle size={20}/> Yeni Sınav</button>
                <button onClick={() => setMode('edit')} className={`flex-1 p-4 flex items-center justify-center gap-2 font-bold transition-colors ${mode === 'edit' ? 'bg-orange-50 text-orange-600 border-b-2 border-orange-600' : 'text-slate-500 hover:bg-slate-50'}`}><Edit size={20}/> Düzenle</button>
            </div>
            <div className="p-8 bg-white border-b border-slate-100 grid md:grid-cols-3 gap-8 items-end">
                {mode === 'create' ? (
                    <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-200 focus-within:border-indigo-500 focus-within:ring-4 focus-within:ring-indigo-500/10 transition-all"><Type size={20} className="text-indigo-500"/><input type="text" placeholder="Örn: 3D Türkiye Geneli" className="w-full bg-transparent outline-none text-sm font-bold text-slate-700 placeholder-slate-400" value={examConfig.name} onChange={e => setExamConfig({...examConfig, name: e.target.value})} /></div>
                    </div>
                ) : (
                    <div className="space-y-2">
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Sınav Seç</label>
                        <div className="flex items-center gap-3 bg-orange-50 p-3 rounded-2xl border border-orange-200"><Search size={20} className="text-orange-500"/><select className="w-full bg-transparent outline-none text-sm font-bold text-slate-700 cursor-pointer" value={selectedExamId} onChange={e => setSelectedExamId(e.target.value)}><option value="">Seçiniz...</option>{uniqueExams.map(name => <option key={name} value={name}>{name}</option>)}</select></div>
                    </div>
                )}
                <div className="space-y-2"><label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Tarih</label><div className="flex items-center gap-3 bg-slate-50 p-3 rounded-2xl border border-slate-200"><Calendar size={20} className="text-indigo-500"/><input type="date" className="w-full bg-transparent outline-none text-sm font-bold text-slate-700" value={examConfig.date} onChange={e => setExamConfig({...examConfig, date: e.target.value})} /></div></div>
                <div className="space-y-2"><label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Tür</label><div className="flex bg-slate-100 p-1 rounded-xl">{['tyt', 'ayt', 'both'].map((type) => (<button key={type} onClick={() => setExamConfig({...examConfig, type})} className={`flex-1 text-xs font-bold py-2.5 rounded-lg transition-all duration-200 ${examConfig.type === type ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>{type === 'both' ? 'TYT + AYT' : type.toUpperCase()}</button>))}</div></div>
            </div>
                ...
                <table className="w-full text-sm text-left">
                    <thead className="text-xs text-slate-500 uppercase bg-slate-50/50 border-b border-slate-100">
                        <tr>
                            ...
                            {(examConfig.type !== 'ayt') && <><th className="px-2 text-center text-indigo-400 font-bold">Mat</th><th className="px-2 text-center text-indigo-400 font-bold">Turk</th><th className="px-2 text-center text-indigo-400 font-bold">Fen</th><th className="px-2 text-center text-indigo-400 font-bold">Sos</th><th className="px-2 text-center w-24 bg-indigo-50/50 text-indigo-700 font-extrabold">TYT P</th></>}
                            {(examConfig.type !== 'tyt') && <><th className="px-2 text-center text-purple-400 font-bold">Mat</th><th className="px-2 text-center text-purple-400 font-bold">Fen</th><th className="px-2 text-center w-24 bg-purple-50/50 text-purple-700 font-extrabold">AYT P</th></>}
            ...
            <div className="p-6 bg-white border-t border-slate-100 flex justify-between items-center fixed bottom-0 right-0 left-0 md:left-64 z-40 shadow-lg-top"><div className="text-sm text-slate-500 font-medium">Toplam <span className="font-bold text-slate-800">{rows.length}</span> öğrenci</div><button onClick={handleSaveAll} disabled={isSaving} className="flex items-center gap-2 px-8 py-4 rounded-2xl font-bold text-white bg-indigo-600 hover:bg-indigo-700 shadow-xl transition-all">{isSaving ? 'İşleniyor...' : <><Save size={20}/> Kaydet</>}</button></div>
        </div>
    );
}