    ...
    <>
        ...
        <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] z-50 px-1 py-1 safe-area-bottom">
            <div className="flex overflow-x-auto gap-2 no-scrollbar pb-1 px-2">
                {filteredMenu.map(item => (
                    <button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex flex-col items-center justify-center min-w-[4rem] p-2 rounded-xl transition-all flex-shrink-0 ${activeTab === item.id ? 'text-indigo-600 bg-indigo-50' : 'text-slate-400'}`}>
                        {item.icon}
                        <span className="text-[9px] font-bold mt-1 truncate w-full text-center">{item.label}</span>
                    </button>
                ))}
                <button onClick={handleLogout} className="flex flex-col items-center justify-center min-w-[4rem] p-2 rounded-xl text-red-400 flex-shrink-0">
        ...
        {/* MOBİL ÜST BAR */}
        <div className="md:hidden fixed top-0 left-0 right-0 bg-white border-b border-slate-100 z-40 px-4 py-3 flex justify-between items-center shadow-sm h-16">
             ...
             <div className="flex items-center gap-2" onClick={() => setActiveTab(currentUser.isAdmin ? 'dashboard' : 'profile')}>
                 <span className="text-xs font-bold text-slate-600 max-w-[100px] truncate">{currentUser.username}</span>
